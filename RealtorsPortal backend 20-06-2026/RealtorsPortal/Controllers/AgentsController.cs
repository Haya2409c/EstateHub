    ...
    public class AgentsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search)
        {
            ...
            var listingCounts = await _db.Properties
                .Where(p => p.SellerId != null && agentIds.Contains(p.SellerId) && p.ApprovalStatus == "Active")
        ...
        // GET: /Agents/Details/{slug}
        public async Task<IActionResult> Details(string slug)
        {
            var agent = await _db.Agents.FirstOrDefaultAsync(a => a.Slug == slug);
            if (agent == null) return NotFound();

            if (!string.IsNullOrEmpty(agent.Email))
            {
                var user = await _db.Users
                    .FirstOrDefaultAsync(u => u.Email == agent.Email && u.AccountType == "Agent");
                if (user != null)
