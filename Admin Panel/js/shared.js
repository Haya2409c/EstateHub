...
document.addEventListener('DOMContentLoaded', () => {
    // 0. Auth guard — runs on every page except login.html
    ...
    // ── Inject logout button into sidebar user mini-profile ──
    const miniProfile = document.querySelector('#sidebar .border-t .flex.items-center');
    if (miniProfile) {
        const logoutBtn = document.createElement('button');
        logoutBtn.setAttribute('data-action', 'logout');
        logoutBtn.title = 'Logout';
        logoutBtn.className = 'ml-auto p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors flex-shrink-0';
        logoutBtn.innerHTML = '<i data-lucide="log-out" class="w-4 h-4"></i>';
        miniProfile.appendChild(logoutBtn);
        if (window.lucide) lucide.createIcons();
    }
});
